...
export const CookingControls: React.FC<CookingControlsProps> = ({
  ...
  const arrowRotation = useDerivedValue(() => {
    return withTiming(isIngredientsOpen ? 180 : 0, { duration: 300 });
  }, [isIngredientsOpen]);
    ...
    >
      <Pressable
        ...
        className={`h-16 w-16 items-center justify-center rounded-2xl bg-white/10 active:scale-95 ${
          currentStep === 0 ? "opacity-20" : "opacity-100"
        }`}
      >
      ...
      <Pressable
        ...
        className={`flex-1 flex-col items-center justify-center gap-1 rounded-2xl border active:scale-95 transition-all ${
          isIngredientsOpen ? "bg-white border-white" : "bg-transparent border-white/30"
        }`}
      >
        <Text
          className={`text-[10px] font-bold uppercase tracking-wider ${
            isIngredientsOpen ? "text-black" : "text-white"
          }`}
        >
          {t("recipe.ingredients")}
        </Text>
        <Animated.View style={arrowStyle}>
          <ArrowUp size={16} color={isIngredientsOpen ? "black" : "white"} />
        </Animated.View>
      </Pressable>
