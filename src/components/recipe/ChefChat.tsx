  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "welcome",
      role: "assistant",
      text: t("recipe.chefChat.welcome", {
        defaultValue:
          "Hi! I'm your AI Chef. I can help you with this recipe. Ask me anything about the ingredients, steps, or techniques!",
      }),
      timestamp: Date.now(),
    },
  ]);
          ...
          <View className="flex-row items-center justify-between border-b border-border-light bg-surface-elevated px-6 py-4">
            <View className="flex-row items-center gap-3">
              <View className="h-10 w-10 items-center justify-center rounded-full bg-primary/10">
                <SparkleIcon size={20} color="#334d43" weight="fill" />
              </View>
              <View>
            ...
            <Pressable
              onPress={onClose}
              className="rounded-full bg-surface-texture-light p-2 active:bg-surface-texture-dark"
            >
              <XIcon size={20} color="#78716c" />
            </Pressable>
          </View>
                ...
                >
                  <Text
                    className={`text-base leading-snug ${
                      msg.role === "user" ? "text-white" : "text-foreground-text"
                    }`}
                  >
                    {msg.text}
                  </Text>
          ...
          >
            <View className="flex-row items-center gap-2 rounded-full border border-border bg-surface-texture-light px-4 py-2">
              <TextInput
                ...
                placeholder={t("recipe.chefChat.placeholder", {
              ...
              <Pressable
                ...
                className={`rounded-full p-2 ${
                  input.trim() ? "bg-primary" : "bg-surface-texture-dark"
                }`}
              >
                <PaperPlaneRightIcon
                  size={20}
                  color={input.trim() ? "white" : "#a8a29e"}
                  weight="fill"
                />
              </Pressable>
            </View>
